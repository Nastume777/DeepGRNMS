...
def generate_multi_data(index):
    ...
    for i in Network_list:
        if i == index:
            continue
        file_name = pre_file + "subdata_" + str(i) + ".csv"
    ...
    for i in range(2):
        ...
        pair_data = generate_pair_data(pair_expression_data)
...
class Config(object):
    def __init__(self, input_size, num_classes):
        ...
        self.embedding_dim = 3                                        # 字向量维度
        self.num_filters = 8
        self.n_hidden = 8
        self.num_classes = num_classes  # 类别数
        self.learning_rate = 1e-3  # 学习率

class Cov_MLP(nn.Module):
    def __init__(self, config):
        super(Cov_MLP, self).__init__()

        self.embedding = nn.Embedding(config.input_size, config.embedding_dim)
        # 1. pair embedding
        self.out_pair = nn.Sequential(
            nn.Linear(config.sent_len, config.n_hidden),
    ...
    def forward_feature(self, x):
        ...
        return self.out_main(output)

    def forward_pair_feature(self, x):
        return self.out_pair(x.to(torch.float))


if __name__ == '__main__':
    Network = 1
    Network_list = [1, 2, 3, 4]
    pre_file = "source/"
    data_file = pre_file + "subdata_" + str(Network) + ".csv"

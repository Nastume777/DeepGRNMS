...
def generate_tf_data(expression_data):
    ...
    for i in range(data_tf.shape[0]):
        row = data_tf[i]
        row = (np.log10(row / len(row) + 10 ** -2) + 2) / 2
        data_tf[i] = np.around(row * 50)
...
def generate_multi_data(data_file_list):
    multi_data = []
...
if __name__ == '__main__':
    ...
    data_file = "subdata_1.csv"
    output_file = "coexpressed_result1.txt"
    other_file = ["subdata_2.csv", "subdata_3.csv"]
    ...
    multi_data = generate_multi_data(other_file)
